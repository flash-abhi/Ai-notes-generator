...
import { downloadPdf } from "../hooks/api";
...
const FinalResult = ({result}) => {
  ...
  return (
    <div className='mt-6 p-3 space-y-10 bg-white'>
        <div className='flex flex-col md:flex-row md:items-center md:justify-between gap-4'>
            ...
            <div className='flex gap-3'>
                <button onClick={() => setQuickRevision(!quickRevision)} className={`
                    px-4 p-2 rounded-lg text-sm font-medium transition ${quickRevision ? "bg-green-600 text-white" : "bg-green-100 text-green-700 hover:bg-green-200"}
                    `}>
        ...
        {quickRevision && <section className="rounded-xl bg-gradient-to-r from-green-50 border border-green-200 p-6">
        ...
        { result.charts?.length >0 &&
            <section>
                <SectionHeader icon="📈" title={"Chart Analysis"} color={"green"} />
        ...
        {result.charts && result.charts.length ===0  && (
            <p className="text-sm text-gray-400 italic">
                📈 Charts are not relevant or required for this topic.
            </p>
        )}
        <section>
